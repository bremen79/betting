...
& \ge \sigma \left(1 - \exp\left(-\frac{\sqrt{\ln d}}{2.6 \sqrt{2\pi}}\right)\right) \left(\sqrt{2 \ln d - 2 \ln \ln d} +\sqrt\frac{2}{\pi}\right) -\sqrt{\frac{2}{\pi}} \sigma \\
& \ge 0.19 \sigma \sqrt{\ln d} - 0.68 \sigma.
\end{align*}
\end{theorem}
...
& = \sigma \left(C\sqrt{\ln d} + \sqrt{\frac{2}{\pi}}\right) \Pr[A] -  \sigma \sqrt{\frac{2}{\pi}} \; .
\end{align*}
...
we can deduce an upper bound on $C(d)$. For any $d \in [2,\infty)$, we have
$$
C(d) \le \max\{C(2), \sqrt{2}\} \le 1.75 \; .
$$
Therefore $C\sqrt{2 \pi} + 2 \le 6.35$ and hence
$$
