...
From the property of the sub-gradient of a convex function we have that, for any sequence of convex functions $\ell_1, \ldots, \ell_n$, vectors $\bw_1,\ldots, \bw_n$, and $\bg_t \in \partial \ell_t(\bw_t)$, we have
\[
Regret^{OCO}_n(\bu) = \sum_{t=1}^n \left( \ell_t(\bw_t) -\ell_t(\bu)\right) \leq \sum_{t=1}^n \langle \bw_t-\bu, \bg_t\rangle = Regret_n(\bu)~.
\]
Hence, the regret w.r.t. to arbitrary convex functions is upper bounded by the linear regret. This means that an \ac{OLO} algorithm can be used to solve an \ac{OCO} problem, just feeding the algorithm with loss vectors $\bg_t$ equal to the subgradients of the functions $\ell_t$.
...
Let $\ell:\fH\rightarrow\R$ a convex function, and fix the vectors $\bg_t$ to be unbiased estimate of the gradient of $\ell$ in $\bw_t$. Then, the following holds
\[
\E\left[\ell\left(\frac{1}{n} \sum_{t=1}^n \bw_t\right)\right] \leq \min_{\bu} \ \ell\left(\bu\right) + \frac{\E[Regret_n(\bu)]}{n}~.
\]
\end{theorem}
...
\begin{equation}
\label{eq:self_tune}
\E\left[\RiskLoss\left(\frac{1}{n} \sum_{t=1}^n \bw_t\right)\right] \leq \scO\left(\inf_{\bu} \min_{\lambda>0} \RiskLoss(\bu) + \lambda \norm{\bu}^2 +\frac{(\ln(n))^{2 \alpha}}{\lambda \, n} \right)~.
\end{equation}
...
where 
\begin{align*}
\hat{\bw}=\argmin_{\bw} \frac{\lambda}{2}\norm{\bw}^2 + \sum_{t=1}^n \ell(\bw_t, z_t)~.
\end{align*}
