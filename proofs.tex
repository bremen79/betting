...
Hence
\[
\wealth_n=\epsilon \prod_{t=1}^n (1+\beta g_t) = \epsilon (1+\beta)^\frac{n+\theta}{2} (1-\beta)^\frac{n-\theta}{2},
\]
where $\theta:=\sum_{t=1}^n g_t$.
It is easy to show that the maximum value of $\wealth_n$ w.r.t. $\beta$ is in $\beta=\frac{\theta}{n}$. 
Hence, we have
\[
\wealth_n = \epsilon \left(1+\frac{\theta}{n}\right)^\frac{n+\theta}{2} \left(1-\frac{\theta}{n}\right)^\frac{n-\theta}{2} 
= \epsilon \left[\left(1+\frac{\theta}{n}\right)^\frac{1+\frac{\theta}{n}}{2} \left(1-\frac{\theta}{n}\right)^\frac{1-\frac{\theta}{n}}{2}\right]^n 
= \epsilon \exp\left( n \, D\left(\frac{1}{2}+\frac{\theta}{2n}\middle\|\frac{1}{2}\right) \right),
...
\[
(1+b_t z_t) \epsilon \exp\left(\frac{x^2}{a (\delta+\sum_{i=1}^{t-1} z_i)}- \sum_{i=1}^{t-1} \frac{z_i}{a (\delta+\sum_{j=1}^{i-1} z_j)}\right)
\geq \epsilon \exp\left(\frac{(x+z_t)^2}{a (\delta+\sum_{i=1}^{t} z_i)}- \sum_{i=1}^{t} \frac{z_i}{a (\delta+\sum_{j=1}^{i-1} z_j)}\right)~.
\]
The above is equivalent to prove
\[
\ln(1+b_t z_t) + \frac{x^2}{a (\delta+\sum_{i=1}^{t-1} z_i)}- \sum_{i=1}^{t-1} \frac{z_i}{a (\delta+\sum_{j=1}^{i-1} z_j)}
\geq \frac{(x+z_t)^2}{a (\delta+\sum_{i=1}^{t} z_i)}- \sum_{i=1}^{t} \frac{z_i}{a (\delta+\sum_{j=1}^{i-1} z_j)}~.
\]

Denote by $\sg_{t-1}=\sum_{i=1}^{t-1} z_i$ and consider the function 
\[
\phi(z)=-\log(1+b_t z) + \frac{(x+z)^2}{a (\sg_{t-1} + |z|)}~.
\]
...
that is
\begin{align*}
\frac{x^2}{a \sg_{t-1}}-\frac{(x+z)^2}{a (\sg_{t-1} +  |z|)} + \log(1+b_t z_t) 
& = \phi(0) - \phi(z) \\
& \geq |z| \left(\phi(0) - \phi(1)\right) \\
&= |z| \left(\frac{x^2}{a \sg_{t-1}} - \frac{(x+1)^2}{a (\sg_{t-1} + 1)} + \log(1+b_t)\right), \forall -1 \leq z\leq 1~.
\end{align*}

Using this relation we have that
\begin{align*}
&-\frac{(x+z_t)^2}{a (\sg_{t-1}+|z_t|)} + \frac{x^2}{a \sg_{t-1}}+\log(1+b_t z_t)-\sum_{i=1}^{t-1} \frac{|z_i|}{a (\sg_{i-1} + 1)} \\
&\qquad \geq |z_t| \left(\frac{x^2}{a \sg_{t-1}} - \frac{(x+1)^2}{a (\sg_{t-1} + 1)} + \log(1+b_t)\right) - \sum_{i=1}^{t-1} \frac{|z_i|}{a (\sg_{i-1} + 1)}\\
...
\[
\frac{x^2}{a \sg_{t-1}(\sg_{t-1} + 1)}-\frac{2 x}{a (\sg_{t-1} + 1)} + \log(1+b_t)
\]
