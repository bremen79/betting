...
Also, the Universal Portfolio algorithm strategy itself cannot be computed in a closed formula and it has to be approximated~\cite{KalaiV03}. Finally, it is not known if the Universal Portfolio algorithm satisfies the Assumption~\ref{assumption:1-d_algo}, hence we cannot use it to solve \ac{OLO}/\ac{OCO} problems in Hilbert spaces.

To overcome these problems, in the next section, we will present a simple algorithm that satisfies Assumption~\ref{assumption:1-d_algo}.
